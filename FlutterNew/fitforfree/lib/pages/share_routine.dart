...
class _ShareRoutinePageState extends State<ShareRoutinePage> {
  ...
  Future<void> deleteMyRoutine(String routineName) async {
    try {
      final accessToken = client.auth.currentSession?.accessToken;
      final response = await http.delete(
        Uri.parse("http://192.227.152.231:3333/Exercise?routineName=$routineName"),
        headers: {
      ...
      if (response.statusCode == 200) {
        debugPrint("Deleted post with ${routineName}");
      } else {
        debugPrint("unsuccessfull${response.statusCode}");
      }
    } catch (e) {
  ...
  Future<void> postData(RoutineDTO routineDTO) async {
    final accessToken = client.auth.currentSession?.accessToken;
    const String apiUrl = 'http://192.227.152.231:3333/Exercise';
